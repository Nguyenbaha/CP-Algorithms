...
struct segment_tree{
    int sum[4 * N], maxpref[4 * N], maxsuff[4 * N], seg[4 * N];
    
    void build(int id, int l, int r){
        if (l == r){
            sum[id] = a[l];
            maxpref[id] = max(0ll, a[l]);
            maxsuff[id] = max(0ll, a[l]);
            seg[id] = max(0ll, a[l]);
            return;
        }
        int mid = (l + r) >> 1;
        ...
        maxpref[id] = max(maxpref[id << 1], sum[id << 1] + maxpref[id << 1 | 1]);
        maxsuff[id] = max(maxsuff[id << 1 | 1], sum[id << 1 | 1] + maxsuff[id << 1]);
        seg[id] = max(max(seg[id << 1], seg[id << 1 | 1]),
                      maxsuff[id << 1] + maxpref[id << 1 | 1]);
    }
    ...
    void update(int id, int l, int r, int i, int val){
        ...
        if (i <= l && r <= i){
            sum[id] = val;
            maxpref[id] = max(0ll, val);
            maxsuff[id] = max(0ll, val);
            seg[id] = max(0ll, val);
            return;
        }
        int mid = (l + r) >> 1;
